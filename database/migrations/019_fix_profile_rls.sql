...
-- Description: Updates the function to properly handle RLS when creating profiles
-- Author: AI Assistant
-- Date: 2025-01-17

-- Drop and recreate the function with proper RLS handling
DROP FUNCTION IF EXISTS public.create_profile_for_user(uuid, text, jsonb);

-- Create the function with proper RLS bypass for profile creation
...
BEGIN
  ...
  -- Create the profile with RLS bypass
  -- Using SECURITY DEFINER means this runs with elevated permissions
...
-- Grant execute permissions to service_role (this is the key!)
...
-- Also update the handle_email_confirmation function to use service_role context
